  ...
  <ul>
    {#if data.session}
      ...
      <form
        ...
        action="/logout"
      >
        <button type="submit" class="secondary outline" style="padding: .5rem"
          >Log out</button
        >
      </form>
    {:else}
...
{#if data.session}
  ...
  >
    <ul>
      <li>
        <h6 style="margin-bottom: 0; margin-right: 3rem">Welcome, {data.profiles[0].full_name}</h6>
      </li>
	  ...
	  <li><a href="/employees" class="secondary"><u>Employees</u></a></li>
    </ul>
  </nav>
{/if}
...
<nav>
  ...
  <li><div><img alt="Theme-switch" id="theme_switcher"></div></li>
</nav>
</footer>
